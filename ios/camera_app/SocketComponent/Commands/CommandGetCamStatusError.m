#import "CommandGetCamStatusError.h"


@interface CommandGetCamStatusError ()
...
@implementation CommandGetCamStatusError
...
- (void)execute
{
    @synchronized (self.app_state) {
        ...
        if ([self.socketConnection sendPacket:&packet] == 0)
        {
            ...
            if (packet.Header.Type == 'R' && packet.data[0] == iCamStatusErrorMsg)
            {
                locIsServerConnected = 1;
                //
                NSString * msg = [NSString stringWithUTF8String:(const char *)&(packet.data[1])];
                self.app_state.bCamStatusError = [msg copy];
            }
        }
